  ...
  <div class="home">
    <div class="home-cont">
      <div class="banner">
        <swiper :options="swiperOption" ref="mySwiper" class="swiper-container">
          ...
          <swiper-slide class="swiper-item" v-for="(item,index) in homedata.banner" :key="index">
             <router-link :to="'/commodity-detail/'+item.bizId">
      ...
      <div class="home-comm-list w">
        ...
        <ul class="new-list">
          <li class="comm-item" v-for="(item,index) in gcdata.goods" :key="index">
            <router-link :to="'/commodity-detail/'+item.id">
              <img alt class="comm-img" :src="item.img.url" lazy="loaded" />
              <div class="info">
      ...
      <div class="home-comm-list w" v-for="(item,index) in normalgoods" :key="index">
		...
		<router-link :to="'/commodity-detail/'+item.goods[0].id" class="link-img">
          <img alt :src="item.goods[0].img.url" lazy="loaded" />
        </router-link>
        <!---->
        <ul class="home-commd-list">
          <!---->
          <li
            class="home-commd-card"
            v-for="(i,idx) in item.goods"
            :key="idx"
            :class="{'last-one':idx%4==0}"
            :style="{display:idx0(idx)}"
          >
            <router-link :to="'/commodity-detail/'+i.id">
              <img alt class="card-img" :src="i.img.url" lazy="loaded" />
              <div class="card-info">
  ...
  methods: {
    ...
    fn() {
      this.$axios({
        ...
        data:
          "cateId=9c43e98e287bd85f66e98b3bb49e87e1&page=1&perPage=28&type=1&uid=&sid="
      }).then(res => {
        console.log(res);
